...
class SimulatedNetworkEnv(gym.Env):
    ...
    def __init__(self,
        ...
        if USE_CWND:
            self.action_space = spaces.Box(np.array([-2, -2]), np.array([2, 2]), dtype=np.float32)
        else:
            self.action_space = spaces.Box(np.array([-2]), np.array([2]), dtype=np.float32)
    ...
    def step2(self, actions):
        ...
        self.reward_sum += reward
        i=2
        #Sprint(str(i)+'-'+str(self.steps_taken))
        if(self.steps_taken==1600):
            self.dump_events_to_file('./data/pcc'+str(i)+'.json')
        return sender_obs, reward, (self.steps_taken >= self.max_steps or should_stop), {}
...
register(id='PccNs-v2'+str(int(USE_CWND))+str(env_num), entry_point='network_sim2'+str(int(USE_CWND))+str(env_num)+':SimulatedNetworkEnv')
