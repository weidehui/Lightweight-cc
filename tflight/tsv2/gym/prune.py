...
def get_args():
    ...
    parser.add_argument('--step-per-epoch', type=int, default=500)  # 1000
    parser.add_argument('--collect-per-step', type=int, default=10)  # 10
    parser.add_argument('--repeat-per-collect', type=int, default=4)
    parser.add_argument('--batch-size', type=int, default=512)
    parser.add_argument('--layer-num', type=int, default=3)
    parser.add_argument('--episode-per-test', type=int, default=2)
...
def test_ppo(args=get_args()):
    ...
    env = DummyVectorEnv(
        [lambda: gym.make('PccNs-v00') for _ in range(args.training_num)])
    ...
    pmodel=prune_network.prune_layer(policy)
    env = gym.make('PccNs-v00')
    pmodel.eval()
    collector = Collector(pmodel, env)
